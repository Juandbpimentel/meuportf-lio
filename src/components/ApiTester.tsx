...
export function ApiTester({ project, onClose, lang, isDark }: ApiTesterProps) {
  ...
  const resolvedPath = useMemo(() => {
    if (!activeEndpoint) return "";
    return (
      activeEndpoint.pathParams?.reduce((acc, param) => {
        return acc.replace(
          `:${param.name}`,
          pathParamValues[param.name] || `:${param.name}`
        );
  ...
  const finalUrl =
    baseUrlInput && resolvedPath ? `${baseUrlInput}${resolvedPath}` : "";
  ...
  const handleSendRequest = async () => {
    ...
    const missingParam = activeEndpoint.pathParams?.find(
      (param) => !pathParamValues[param.name]
    );
    if (missingParam) {
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-[1.4fr_0.6fr]">
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setActiveEndpointIndex(Number(e.target.value))
                  }
                  className={cn(
                    "w-full rounded-lg border px-3 py-2 text-sm",
                    isDark
                ...
                >
                  ...
                  >
                    ...
                    <p className="font-mono break-all">
                      {finalUrl || `${baseUrlInput}${activeEndpoint.path}`}
                    </p>
                  </div>
            ...
            <div className="space-y-3">
              ...
              <button
                ...
                className={cn(
                  "btn btn--sm justify-center disabled:opacity-60",
                  isDark
                    ? "bg-[#EB8F3E] text-[#1f1307] hover:bg-[#d87a28]"
                    : "bg-[#EB8F3E] text-white hover:bg-[#c45f17]"
                )}
              >
              ...
              {responsePreview && (
                <div
                  className={cn(
                    "space-y-1 rounded-xl border px-3 py-2",
                    isDark
                      ? "border-slate-800 bg-slate-900/70"
                      : "border-slate-200 bg-white"
                  )}
