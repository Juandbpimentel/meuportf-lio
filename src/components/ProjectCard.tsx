...
export function ProjectCard({ project, lang, isDark, onOpenEndpoints }: ProjectCardProps) {
  ...
  const projectType = getLocalized(project.type as string | Record<Lang, string>, lang)
  const projectDescription = getLocalized(project.description as string | Record<Lang, string>, lang)
    ...
    >
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <h3 className={cn('text-lg font-semibold', isDark ? 'text-slate-50' : 'text-slate-900')}>{project.title}</h3>
        </div>
        {project.isInteractive && (
          <span
            className={cn(
              'inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-semibold',
              isDark ? 'bg-[rgba(235,143,62,0.12)]' : 'bg-[rgba(235,143,62,0.18)]'
            )}
            data-primary
          >
      ...
      <p className={cn('mt-2 text-sm', isDark ? 'text-slate-300' : 'text-slate-700')}>{projectDescription}</p>
      ...
      <div className={cn('mt-4 flex flex-wrap items-center gap-3 text-sm', isDark ? 'text-slate-300' : 'text-slate-700')}>
        <a
          className={cn('btn',
        ...
        {project.isInteractive && project.endpoints?.length ? (
          ...
          >
            <ServerCog className="icon h-4 w-4 transition-transform group-hover:scale-110 group-hover:text-(--primary-strong)" /> {labels[lang].endpoints}
          </button>
        ) : null}
        {project.isInteractive && (
          <span
          ...
          >
            {labels[lang].status}:
            {(statusQuery.isPending || statusQuery.isFetching) && <span className="text-amber-400">Verificando...</span>}
            {statusQuery.isSuccess && (
              <span
                className={cn(
                  statusQuery.data.status === 'online'
                    ? (isDark ? 'status-online-dark font-semibold' : 'font-semibold text-[#a04700]')
                    : 'text-rose-400'
                )}
              >
            ...
            {statusQuery.isError && <span className="text-rose-400">offline</span>}
          </span>
        )}
      </div>
    </motion.div>
